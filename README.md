...
> Примечание: файл `test_delete_tack_ui.py` сейчас назван с опечаткой (`tack` вместо `task`). На работу проекта это не влияет, но позже его можно переименовать для аккуратности.
...
Сейчас `requirements.txt` покрывает backend/API-зависимости. Для UI-тестов отдельно установи Playwright:
...
- привести `README`, зависимости и структуру проекта к полностью синхронному состоянию;
- добить и стабилизировать весь набор UI-автотестов;
- вынести повторяющиеся тестовые хелперы в общие фикстуры;
- добавить более явные инструкции по запуску UI-тестов;
